---
name: image-change-action
description: "两步调用换动作工作流：先运行 prompt workflow 生成 final_prompt 和标准化输入，再把结果作为 input_data 传给图片生成 workflow。Use when changing the pose/action of a model by chaining workflow IDs 69cb41fbd03ef955b10c1d9a and 694e206fc1c0b24dc831ad8b with wf run."
---

# Image Change Action

这个 skill 用于串联两个远程 workflow，完成「换动作」图片生成：

1. `69cb41fbd03ef955b10c1d9a`：先根据人物图和参考动作图，产出带 `final_prompt` 的中间结果。
...
- Workflow ID: `69cb41fbd03ef955b10c1d9a`
- 作用：生成中间 dataframe，并补出后续生成需要的 `final_prompt`

### 可覆盖输入

- `--case`
  - 业务场景。这个 skill 固定使用 `change-action`。
  - 除非你明确要切到别的 sheet，否则不要改。
- `--product_image_url`
  - 人物图片 URL（单张）。保留人物外观，换动作。
- `--reference_image_url`
  - 参考动作图 URL（数组）。上传几张出几张，会展开成多行。
- `--aspect_ratio`
  - 生成比例。未知时用 `auto`。
- `--resolution`
  - 清晰度。可用值按当前 workflow 描述为 `1K` / `2K`，未知时用 `2K`。
- `--user_description`
  - 可选文本补充，用于微调模特特征（非必填，不能自定义出图张数）。
- `--llm_model`
...
```sh
wf run 69cb41fbd03ef955b10c1d9a \
  --case "change-action" \
  --product_image_url "<PERSON_IMAGE_URL>" \
  --reference_image_url '["<ACTION_REFERENCE_URL>"]' \
  --aspect_ratio "auto" \
  --resolution "2K" \
  --user_description ""
```
...
wf run 694e206fc1c0b24dc831ad8b \
  --input_data '[{"case":"change-action","product_image_url":"<PERSON_IMAGE_URL>","reference_image_url":"<ACTION_REFERENCE_URL>","aspect_ratio":"auto","resolution":"2K","user_description":"","image_count":1,"final_prompt":"<FINAL_PROMPT_FROM_STEP_1>"}]'
```
...
1. 先运行第一步 workflow `69cb41fbd03ef955b10c1d9a`，生成带 `final_prompt` 的中间 dataframe。
...
- 这个 skill 固定围绕 `case=change-action` 工作，不要把它当成通用 case 切换器。
