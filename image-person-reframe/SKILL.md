...
description: "使用 Fastest AI 的 `POST /v1/tool/image/person_reframe` 接口做人物构图对齐。Use when reframing a target image to match the composition of a reference image by calling the Fastest AI person_reframe API with curl."
---
...
curl -X POST 'https://api.fastest.ai/v1/tool/image/person_reframe' \
  ...
  -d '{
    "ref": "<COMPOSITION_REFERENCE_IMAGE_URL>",
    "target": "<INPUT_IMAGE_URL>"
  }'
```
...
curl -X POST 'https://api.fastest.ai/v1/tool/image/person_reframe' \
  ...
  -d '{
    ...
    "target": "https://example.com/source.jpg"
  }'
```
