---
name: image-change-background
description: "两步调用换背景工作流：先运行 prompt workflow 生成 final_prompt 和标准化输入，再把结果作为 input_data 传给图片生成 workflow。Use when changing the background of an image by chaining workflow IDs 69cb69222a1008a8834cfe60 and 694e206fc1c0b24dc831ad8b with wf run."
---

# Image Change Background

这个 skill 用于串联两个远程 workflow，完成「换背景」图片生成：

1. `69cb69222a1008a8834cfe60`：先根据输入图片和参考背景图，产出带 `final_prompt` 的中间结果。
...
- Workflow ID: `69cb69222a1008a8834cfe60`
- 作用：生成中间 dataframe，并补出后续生成需要的 `final_prompt`

### 可覆盖输入

- `--case`
  - 业务场景。这个 skill 固定使用 `change-background`。
  - 除非你明确要切到别的 sheet，否则不要改。
- `--product_image_url`
  - 输入原图 URL。应当是需要保留主体的图片。
- `--reference_image_url`
  - 参考背景图 URL。
  - 注意：workflow 元数据里这个字段的旧描述仍写着“参考动作”，但结合该 workflow 的标题、任务和 `case=change-background` 可推断，在这个 skill 里它应当作为背景参考图使用。
- `--aspect_ratio`
  - 生成比例。未知时用 `auto`。
- `--resolution`
  - 清晰度。可用值按当前 workflow 描述为 `1K` / `2K`，未知时用 `2K`。
- `--user_description`
  - 可选文本补充，用于微调人物或模特特征。
- `--llm_model`
...
第一步当前还会带出 `user_description`。这个额外字段可以保留，不影响后续理解。

### 示例命令

```sh
wf run 69cb69222a1008a8834cfe60 \
  --case "change-background" \
  --product_image_url "<SOURCE_IMAGE_URL>" \
  --reference_image_url "<BACKGROUND_REFERENCE_URL>" \
  --aspect_ratio "auto" \
  --resolution "2K" \
  --user_description ""
```
...
wf run 694e206fc1c0b24dc831ad8b \
  --input_data '[{"case":"change-background","product_image_url":"<SOURCE_IMAGE_URL>","reference_image_url":"<BACKGROUND_REFERENCE_URL>","aspect_ratio":"auto","resolution":"2K","user_description":"","image_count":1,"final_prompt":"<FINAL_PROMPT_FROM_STEP_1>"}]'
```
...
1. 先运行第一步 workflow `69cb69222a1008a8834cfe60`，生成带 `final_prompt` 的中间 dataframe。
...
- 这个 skill 固定围绕 `case=change-background` 工作，不要把它当成通用 case 切换器。
