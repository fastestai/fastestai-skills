---
name: image-change-product
description: "两步调用换商品工作流：先运行 prompt workflow 生成 final_prompt 和标准化输入，再把结果作为 input_data 传给图片生成 workflow。Use when changing the product in a scene image by chaining workflow IDs 694cb498c1c0b24dc82f4264 and 694e206fc1c0b24dc831ad8b with wf run."
---

# Image Change Product

这个 skill 用于串联两个远程 workflow，完成「换商品」图片生成：

1. `694cb498c1c0b24dc82f4264`：先根据新商品图和旧商品场景图，产出带 `final_prompt` 的中间结果。
...
- Workflow ID: `694cb498c1c0b24dc82f4264`
- 作用：生成中间 dataframe，并补出后续生成需要的 `final_prompt`

### 可覆盖输入

- `--case`
  - 业务场景。这个 skill 固定使用 `change-product`。
  - 除非你明确要切到别的 sheet，否则不要改。
- `--product_image_url`
  - 新商品图 URL（数组）。上传几张出几张。
- `--reference_image_url`
  - 旧商品的场景图 URL。用于提供原始场景和模特信息。
- `--aspect_ratio`
  - 生成比例。未知时用 `auto`。
- `--resolution`
...
```sh
wf run 694cb498c1c0b24dc82f4264 \
  --case "change-product" \
  --product_image_url '["<NEW_PRODUCT_IMAGE_URL>"]' \
  --reference_image_url "<SCENE_IMAGE_URL>" \
  --aspect_ratio "auto" \
  --resolution "2K" \
  --user_description ""
```
...
wf run 694e206fc1c0b24dc831ad8b \
  --input_data '[{"case":"change-product","product_image_url":"<NEW_PRODUCT_IMAGE_URL>","reference_image_url":"<SCENE_IMAGE_URL>","aspect_ratio":"auto","resolution":"2K","user_description":"","image_count":1,"final_prompt":"<FINAL_PROMPT_FROM_STEP_1>"}]'
```
...
1. 先运行第一步 workflow `694cb498c1c0b24dc82f4264`，生成带 `final_prompt` 的中间 dataframe。
...
- 这个 skill 固定围绕 `case=change-product` 工作，不要把它当成通用 case 切换器。
